...
//         // 🟢 Store individual user details using sessionStorage
//         sessionStorage.setItem("name", data.user.name);
//         sessionStorage.setItem("role", data.user.role);
//         sessionStorage.setItem("userId", data.user._id);

//         setTimeout(() => {
//           navigate("/houserental"); // Redirect to house rental page
//         }, 1000);
//       } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          navigate("/houserental", { replace: true }); // ✅ Prevent back to signup
        }, 1000);
      } else {
        setMessage(`❌ ${data.message || "Error registering user."}`);
      }
